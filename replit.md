...
- **Global Label Audit** (`server/labelAudit.ts`): Runs on EVERY response in `buildResponse()` after allergen post-check. Infers correct labels from recipe content (ingredients + steps): `meal_style` (21 style evidence patterns), `cuisine` (13 cuisine indicator sets, 2+ unique match threshold), `cooking_method` (9 methods), `base_carb` (13 carb types), `healthiness` (lean/balanced/comfort heuristic scoring), `budget_level` (low/standard/splurge heuristic scoring). Also validates `timing` coherence (prep+cook vs total), `appliance_fit` (recipe vs selected appliances), and `allergen_clean` (re-scan post-substitution). Auto-corrects mismatched labels, writes inferred values back to recipe fields, rebuilds title when meal_style or cuisine changes. Improved `inferIngredientCategory()` with 7 categories (protein, produce, dairy, grain, spice, sauce, pantry) plus override patterns for compound terms (e.g., "black pepper" → spice not produce, "olive oil" → sauce, "chicken broth" → pantry). Debug UI (`?debug=1`) shows full audit details: requested vs inferred vs final labels, fixes applied, and issues found. Server logs `[audit]` line per request with all label decisions.
