...
- **Cuisine Guard** (`server/cuisine-guard.ts`): A 7th validation check in `v2-validator.ts`. For each selected cuisine (cajun, mexican, italian, asian, korean, thai, indian, middle_eastern, bbq, canadian) a per-cuisine ingredient blocklist identifies cross-cuisine contamination (e.g. soy sauce / sesame oil in a Cajun recipe). Each detected violation deducts 2 points from a starting score of 10. Score < 6 rejects the candidate with reason `cuisine-mismatch:cuisine=X,score=Y`. Score 6–7 logs a warning but passes. Score ≥ 8 is a clean pass. `cuisine_style="any"` always scores 10 (no enforcement). `cuisineMatchScore` is logged for every validated candidate.
