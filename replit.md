...
- **Explore Recipes (Spoonacular)**: Search/discovery page at `/explore` powered by Spoonacular API. Backend service in `server/spoonacular.ts` with `searchRecipes()` and `getRecipeById()`. Routes: GET `/api/explore/search` (query, cuisine, diet, intolerances, excludeIngredients, includeIngredients, equipment, maxReadyTime, minServings, maxServings) and GET `/api/explore/recipe/:id` (full recipe detail with macros, ingredients, steps). Frontend page at `client/src/pages/explore.tsx` uses Firehall filters (cuisine style, meal format, proteins, allergens, appliances, time, crew size, pantry mode) mapped to Spoonacular params: cuisine→cuisine, allergens→intolerances+excludeIngredients, time→maxReadyTime, appliances→equipment, pantry→includeIngredients, proteins→query keywords, vegetarian→diet=vegetarian, meal format→query keywords (bowl/burger/pasta/etc.), crew size→minServings/maxServings. In-memory cache (10min TTL, 200 entries). Returns 503 if SPOONACULAR_API_KEY not set.
