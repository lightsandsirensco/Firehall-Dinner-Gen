...
export function sanitizeGenerateRequest(request: GenerateRequest): GenerateRequest {
  return {
    ...
    // Canonical-key sanitization only — entitlement gating (Pro-only) happens
    // separately in server/routes.ts, which has access to the request's auth
    // context that this pure function does not.
    foods_to_avoid: sanitizeFoodPreferenceKeys(request.foods_to_avoid),
