...
const ALLERGEN_SAFE_REPLACEMENTS: AllergenSafeReplacement[] = [
  ...
  { match: /rice or pasta/i, safeFor: ["gluten"], replacement: { item: "Chickpea pasta (gluten-free)", amount: "1 lb", baseQty: 1, unit: "lb" }, stepFind: /rice or pasta/gi, stepReplace: "gluten-free pasta" },
  { match: /pasta/i, safeFor: ["gluten"], replacement: { item: "Chickpea pasta (gluten-free)", amount: "1 lb", baseQty: 1, unit: "lb" }, stepFind: /pasta/gi, stepReplace: "gluten-free pasta" },
...
const PROTEIN_INGREDIENTS: Record<string, RawIngredient[]> = {
  chicken: [
    ...
    { item: "Mixed greens or extra vegetables", amount: "2 cups", baseQty: 2, unit: "cups" },
  ...
  beef: [
    ...
    { item: "Mixed vegetables (extra)", amount: "2 cups", baseQty: 2, unit: "cups" },
...
const VEG_FALLBACK_SETS: VegFallbackSet[] = [
  {
    ...
    steps: [
      ...
      { heading: "Combine and serve (no heat, 2 min)", body: "Serve seasoned chickpeas and vegetables in bowls. Each serving packs protein from the chickpeas — hearty and filling." },
    ],
  ...
  },
  {
    ...
    steps: [
      ...
      { heading: "Finish and serve (no heat, 2 min)", body: "Squeeze lime juice over the beans and top with fresh cilantro. Serve in bowls — packed with fiber and plant protein." },
    ],
    ...
    whySnippet: "black beans and peppers",
  },
  {
    base: "kidney beans",
    ingredients: [
    ...
    steps: [
      ...
      { heading: "Serve (no heat, 2 min)", body: "Spoon into bowls. Each bowl delivers hearty protein and a rich, smoky flavor." },
    ],
  ...
  },
  {
    ...
    steps: [
      ...
      { heading: "Glaze and serve (medium, 2 min)", body: "Pour the glaze over crispy tempeh and toss to coat. Cook 1 minute until sticky. Serve with steamed broccoli. Top with sesame seeds." },
    ],
    ...
    whySnippet: "glazed tempeh with broccoli",
  },
  {
    base: "white beans",
    ingredients: [
...
const STOVETOP_STEPS: Record<string, { heading: string; body: string }[]> = {
  chicken: [
    ...
    { heading: "Combine and serve (no heat, 2 min)", body: "Return chicken to the pan, toss everything together to coat. Plate and serve. Internal temp check: chicken should read 165°F in the thickest piece." },
  ],
  beef: [
    ...
    { heading: "Plate and serve (no heat, 2 min)", body: "Spoon the seasoned beef mixture into bowls. Top with shredded cheese while hot so it melts. Serve family-style." },
  ],
  pork: [
  ...
  turkey: [
    ...
    { heading: "Assemble the bowls (no heat, 3 min)", body: "Spoon seasoned turkey and black bean mixture into bowls." },
  ...
  fish: [
    ...
    { heading: "Plate and serve (no heat, 2 min)", body: "Place salmon fillets on plates alongside asparagus and vegetables. Squeeze fresh lemon over the fish and serve with lemon wedges." },
  ],
};
...
const OVEN_STEPS: Record<string, { heading: string; body: string }[]> = {
  ...
  vegetarian: [
    ...
    { heading: "Combine and serve (no heat, 2 min)", body: "Return crispy tofu to the pan, toss gently. Serve in bowls. Each serving packs protein from both tofu and chickpeas." },
  ],
};
...
function resolveBaseCarb(archetype: FallbackArchetype, structureType?: StructureType): string {
  ...
  if (archetype.baseCarb === "rice") {
    if (struct === "rice-bake") return "rice";
    if (NO_BASE_CARB_STRUCTURES.has(struct)) return "none";
...
export function buildFallbackRecipe(
  ...
  if (request.vegetarian_swap_needed && !isVegetarian) {
    vegOption = {
      ...
      plating_notes: `Plate the tofu portion separately on the same sides and vegetables. Label clearly so the vegetarian crew member can grab theirs easily.`,
    };
  }
